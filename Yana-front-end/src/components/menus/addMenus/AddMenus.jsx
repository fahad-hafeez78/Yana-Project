...
const AddMenus = () => {
  ...
  const { mealsData, existingmenusData } = location.state || {}; // Destructure the passed data
    ...
    if (name === "MenuName") {
      ...
      if (mealsData) {
        const filtered = mealsData.filter((meal) =>
          meal.title && meal.title.toLowerCase().includes(value.toLowerCase())
        );
  ...
  const handleAddToMenu = () => {
    ...
    const { _id, title, image } = selectedMeal;
  
    if (menuData?.meals.some((meal) => meal._id === _id)) {
      alert("This meal is already in the menu.");
      setMealInput('');
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!menuData.meals.length) {
      dispatch(showErrorAlert('Please add dishes to the menu.'));
      return;
    }
    ...
    // Append each dish
    menuData.meals.forEach((dish, index) => {
      formData.append(`meals[${index}][_id]`, dish._id);
      // formData.append(`meals[${index}][name]`, dish.name);
      // formData.append(`meals[${index}][imgPath]`, dish.imgPath);
    });
