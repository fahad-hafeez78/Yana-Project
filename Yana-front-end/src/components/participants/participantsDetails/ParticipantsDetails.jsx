...
const ParticipantsDetails = () => {
  ...
  const { memberData: data } = location.state || {}; // Destructure the passed data
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const fieldsToSanitize = [
        "MCPT", "MAuthUnitsApproved", "MFrequency", "PCPT", "PAuthUnitsApproved", "PFrequency"
      ];
  
      // Create a copy of formData to avoid mutating the state directly
      const sanitizedFormData = { ...formData };
  
      // Sanitize fields in the formData
      ...
      // Dispatch action to update customer with sanitized form data
      await dispatch(customersMiddleware.UpdateCustomer(data._id, sanitizedFormData));
  
      // Navigate to the previous page (dashboard or wherever required)
      navigate(-1); 
    ...
    <div className="bg-white p-5 rounded-2xl">
      <div className="flex flex-col font-['Poppins',sans-serif] gap-4">
        ...
        <div className='flex justify-between'>
          <h1 className="text-[#959595] font-bold text-2xl">Client Details</h1>
      ...
      {isModalOpen && <ParticipantsStatusModal formData={formData} onCancel={() => setisModalOpen(false)} onConfirm={onConfirm} />}
    </div>
  );
};
