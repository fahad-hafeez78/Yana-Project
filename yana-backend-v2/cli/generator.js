...
module.exports = {
    ...
    const controllerContent = `const { status } = require('http-status');  
...
const create${capital} = catchAsync(async (req, res) => {
    ...
    res.status(status.CREATED).send({ success: true, message: "${capital} Created Success", ${name}});
});
  
const get${capital}s = catchAsync(async (req, res) => { 
    const ${name}s = await ${name}Service.query${capital}s();
    res.status(status.OK).send({ success: true, ${name}s });
});

const get${capital} = catchAsync(async (req, res) => {
    const ${name} = await ${name}Service.get${capital}ById(req.params.id);
    res.status(status.OK).send({ success: true, ${name} });
});
  
const update${capital} = catchAsync(async (req, res) => {
    const ${name} = await ${name}Service.update${capital}ById(req.params.id, req.body);
    res.status(status.OK).send({ success: true, message: '${capital} Updated Success', ${name} });
});
  
const delete${capital} = catchAsync(async (req, res) => {
    await ${name}Service.delete${capital}ById(req.params.id);
    res.status(status.OK).send({ success: true, message: "${capital} Deleted Success" });
});
...
`;
    ...
    const servicesContent = `const { status } = require('http-status');
const ApiError = require('../utils/ApiError');
const { ${capital} } = require('../models'); 


const create${capital} = async (body) => { 
    return await ${capital}.create(body);
};
...
const get${capital}ById = async (id) => {
    let ${name} = await ${capital}.findById(id);
    if (!${name}) {
      throw new ApiError(status.NOT_FOUND, '${name} not found');
    } 
    return ${name};
};
...
`;
        ...
        if(questions.file1 && questions.file1 == `${name}.route.js`){ 
            const filePath = path.join(__dirname, '../src/routes/v2', `${name}.route.js`);
            if (!fs.existsSync(filePath)) {  
                fs.writeFileSync(path.join(__dirname, '../src/routes/v2', `${name}.route.js`), routeContent);
                newFile = true;
            } else { 
                console.log(`\x1b[31m ${name}.route.js file already exist in routes/v2 \x1b[0m`);
            }
        }
